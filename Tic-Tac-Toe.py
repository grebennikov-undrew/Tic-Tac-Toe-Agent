...
class Field:
    ...
    def win_state(state):
            ...
            if (int(state[4])>0 and (int(state[0]) == int(state[4]) and int(state[8]) == int(state[4]) or int(state[2]) == int(state[4]) and int(state[6])==int(state[4]) or int(state[3]) == int(state[4]) and int(state[3]) == int(state[5]) or int(state[1]) == int(state[4]) and int(state[1]) == int(state[7]))):
              return(int(state[4]))
            # все выигрыши через левую верхнюю клетку [0])[0]) (2 шт)
            elif (int(state[0])>0 and (int(state[0]) == int(state[1]) and int(state[0]) == int(state[2]) or  int(state[0]) == int(state[3]) and int(state[0]) == int(state[6]))):
              return(int(state[0]))
            # все выигрыши через правую нижнюю клетку [2])[2]) (2 шт)
            elif (int(state[8])>0 and (int(state[6]) == int(state[7]) and int(state[6]) == int(state[8]) or int(state[2]) == int(state[5]) and int(state[2]) == int(state[8]))):
              return (int(state[8]))
    ...
    # Обучение агента (игра с другим агентом), количество обучающих игр - 100 по умолчанию
    ...
    def human_decission(self,state,human_number):
        ...
        human_step = input("Куда ходить? Введите число: ")
...
### PVE ###
while True:
    human_chouse = input("За кого будете играть? Введите X или O (английские буквы): ")
    if human_chouse == 'X':
        tic_tac_toe.start_game(player2, human_chouse)
    elif human_chouse == 'O':
        tic_tac_toe.start_game(player1, human_chouse)
    else:
        "Введите правильную букву"
    next_game = input("Попробуем ещё раз? Введите Y или N (английские буквы): ")
    if next_game == 'N':
        break;
